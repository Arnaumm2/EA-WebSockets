...
export class MensajeService {
    ...
    public inicializarSockets(): void {
        this.io.on('connection', (socket: Socket) => {
            ...
            socket.on('nuevo_usuario', (username: string) => {
    console.log('🔥 BACKEND RECIBE nuevo_usuario:', username);

    this.usuariosConectados.set(socket.id, username);

    console.log('MAP:', this.usuariosConectados);

    this.io.emit(
        'usuarios_activos',
        Array.from(this.usuariosConectados.values())
    );
});
